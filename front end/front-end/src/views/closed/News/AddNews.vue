    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-sm">
      ...
      <form @submit.prevent="submitForm" class="space-y-4" enctype="multipart/form-data">
        ...
        <div>
          ...
          <select v-model="form.category" class="border border-gray-300 rounded-lg px-4 py-2 text-sm w-full sm:max-w-xs">
  ...
  methods: {
    ...
    async submitForm() {
      try {
        ...
        if ("Add" === "Add") {
          await this.$apiPost("/news", formData, headers);
          this.$root.$refs.toast.showToast('Added successfully', 'success');
        } else {
          await this.$apiPut("/news/" , this.data.id, formData, headers);
